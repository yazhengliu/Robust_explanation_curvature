...
class ExplainerBase(nn.Module):
    ...
    def control_sparsity(self, mask: Tensor, sparsity=None, **kwargs):
        ...
        if not self.explain_graph:
            ...
            #print('self.hard_edge_mask',self.hard_edge_mask)
...
class WalkBase(ExplainerBase):
    ...
    def extract_step(self, x: Tensor, edge_index: Tensor, edge_weight: Tensor,detach: bool = True, split_fc: bool = False):
        ...
        # print('layer_extractor',layer_extractor)
        ...
        for layer in layer_extractor:
            #print('verify',layer[0],isinstance(layer[0], GNNPool), isinstance(layer[0], MessagePassing))
        ...
        #     print( 'ceshi',walk_step['module'])
        # 
        # for fc_steps in fc_steps:
        #     print('ceshi fc', fc_steps['module'])
        ...
        # print('split_fc',split_fc)
        # print('step',step)
        # if split_fc:
        #     if step['module']:
        #         print('step module',step['module'][0])
        #         fc_steps.append(step)
        # 
        #     for fc_steps in fc_steps:
        #         print('final fc', fc_steps['module'])
    ...
    def eval_related_pred(self, x: Tensor, edge_index: Tensor, masks: List[Tensor], **kwargs):
        ...
        for label, edge_mask in enumerate(masks):
            ...
            ori_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'))

            for mask in self.edge_mask:
                mask.data = edge_mask
            masked_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'))

            # mask out important elements for fidelity calculation
            for mask in self.edge_mask:
                mask.data = 1.0 - edge_mask
            maskout_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'))

            # zero_mask
            for mask in self.edge_mask:
                mask.data = torch.zeros(edge_mask.size(), device=self.device)
            zero_mask_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'))
    ...
    def explain_edges_with_loop(self, x: Tensor, walks: Dict[Tensor, Tensor], ex_label):
        ...
        walks_ids = walks['ids']
        walks_score = walks['score'][:walks_ids.shape[0]].reshape(-1)
...
class FlowBase(LayerEdgeBase):
    ...
    def explain_edges_with_loop(self, x: Tensor, walks: Dict[Tensor, Tensor], ex_label):
        ...
        # print('walks_ids',walks_ids,'walks_score',walks_score)
        if walks_ids.max() <= self.num_edges - 1:  # num_edges includes the self-loop
            idx_ensemble = torch.cat([(walks_ids == i).int().sum(dim=1).unsqueeze(0) for i in range(self.num_edges)], dim=0)
        else:
            idx_ensemble = torch.cat([(walks_ids == i).int().sum(dim=1).unsqueeze(0) for i in range(self.num_edges + self.num_nodes)], dim=0)
