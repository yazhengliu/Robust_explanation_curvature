...
from .base_explainer_link import WalkBase

EPS = 1e-15


class GNN_LRP_link(WalkBase):
    r"""
    ...
    def forward(self,
        ...
        walk_steps, fc_steps = self.extract_step(x, edge_index, kwargs.get('edge_weight'),kwargs.get('pos_edge_index'),detach=False, split_fc=True,)

        print('walk_steps',len(walk_steps))
        print('fc_steps', len(fc_steps))
        for walk_step in walk_steps:
            print('walk module',walk_step['module'])
        for fc_step in fc_steps:
            print('fc module', fc_step['module'])
        ...
        print('walk_indices_list',len(walk_indices_list))
        ...
        if kwargs.get('walks'):
            ...
            def compute_walk_score():
                ...
                for index in range(len(walk_indices_list)):
                    walk_indices=walk_indices_list[index]
                    if index %1000==0:
                        print('index',index)
                    ...
                    for i, fc_step in enumerate(fc_steps):
                        ...
                        if i==1:
                            ...
                            # print('h',h[0],h[1])
                            h=torch.cat((h[kwargs.get('goal_1')], h[kwargs.get('goal_2')]), dim=0)
                            # print(h_new)
                            ...
                            std_h=GraphSequential(*modules)(h)
                            s = fc_gamma_modules[i](h)
                            ht = (s + epsilon) * (std_h / (s + epsilon)).detach()
                            h = ht
