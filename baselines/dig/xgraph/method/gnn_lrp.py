...
class GNN_LRP(WalkBase):
    ...
    def forward(self,
        ...
        labels = [kwargs.get('given_class')]
        self.model.eval()

        walk_steps, fc_steps = self.extract_step(x, edge_index,edge_weight=kwargs.get('edge_weight'), detach=False, split_fc=True)

        # print('walk_steps',len(walk_steps))
        # print('fc_steps', len(fc_steps))
        ...
        # print('walk_indices_list',walk_indices_list)
        ...
        if kwargs.get('walks'):
            ...
            def compute_walk_score():
                ...
                #print('ori_gnn_weights',ori_gnn_weights)
                ...
                for walk_indices in walk_indices_list:
                    ...
                    for i, walk_step in enumerate(walk_steps):
                        ...
                        if hasattr(modules[0], 'nn'):
                            ...
                            std_h = GraphSequential(*modules)(h, edge_index,kwargs.get('edge_weight'))

                            # --- LRP-gamma ---
                            p = gnn_gamma_modules[i](h, edge_index,kwargs.get('edge_weight'))
                    ...
                    # print('debug',torch.zeros(h.shape[0], dtype=torch.long, device=self.device))
                    # 
                    for i, fc_step in enumerate(fc_steps):
                        modules = fc_step['module']
                        #print('fc modules',modules[0])
                        std_h = nn.Sequential(*modules)(h) if i != 0 \
                            else GraphSequential(*modules)(h, torch.zeros(h.shape[0], dtype=torch.long, device=self.device),kwargs.get('edge_weight'))

                        # --- gamma ---
                        s = fc_gamma_modules[i](h) if i != 0 \
                            else fc_gamma_modules[i](h, torch.zeros(h.shape[0], dtype=torch.long, device=self.device),kwargs.get('edge_weight'))
                        ht = (s + epsilon) * (std_h / (s + epsilon)).detach()
                        h = ht
                    
                    # print('h',h[node_idx][label])
        ...
        with torch.no_grad():
            with self.connect_mask(self):
                ...
                for ex_label in ex_labels:
                    ...
                    #edge_masks.append(edge_mask.sigmoid())
