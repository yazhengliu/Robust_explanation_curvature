...
class WalkBase(ExplainerBase):
    ...
    def extract_step(self, x: Tensor, edge_index: Tensor,edge_weight: Tensor, pos_edge_index: Tensor, detach: bool = True, split_fc: bool = False):
        ...
        pred = self.model(x, edge_index,edge_weight,pos_edge_index)
    ...
    def eval_related_pred(self, x: Tensor, edge_index: Tensor, masks: List[Tensor], **kwargs):
        ...
        for label, edge_mask in enumerate(masks):
            ...
            ori_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'),pos_edge_index=kwargs.get('pos_edge_index'))

            for mask in self.edge_mask:
                mask.data = edge_mask
            masked_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'),pos_edge_index=kwargs.get('pos_edge_index'))

            # mask out important elements for fidelity calculation
            for mask in self.edge_mask:
                mask.data = 1.0 - edge_mask
            maskout_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'),pos_edge_index=kwargs.get('pos_edge_index'))

            # zero_mask
            for mask in self.edge_mask:
                mask.data = torch.zeros(edge_mask.size(), device=self.device)
            zero_mask_pred = self.model(x=x, edge_index=edge_index, edge_weight=kwargs.get('edge_weight'),pos_edge_index=kwargs.get('pos_edge_index'))
